...
from .models import subjectLessons, subjectPictures, history, historyPics
...
historyLesson_views = Blueprint('historyLesson_views', __name__)
...
@historyLesson_views.app_template_filter('b64encode')
def base64_encode_filter(binary_data):
    return to_base64(binary_data)


@historyLesson_views.route('/histViewLesson/<int:lesson_number>', methods=['GET', 'POST'])
@cache.cached(timeout=900, key_prefix='histViewLesson')
@login_required
def histViewLesson(lesson_number):
    """Retrieve data from tables"""
    if request.method == 'POST':
        """Get the submitted form data"""
        form_data = request.form

        """Create a list to store history entries"""
        history_entries = []

        """Process the form data and populate history entries"""
        for key, value in form_data.items():
            if key.startswith('response_'):
                ...
                filtered_hist_rows = [row for row in adminSubRow if row['subQuestion'] == question and row['subSubjectCode'] == 'HIS']

                """History table"""
                for lesson_data in filtered_hist_rows:
                    sub_topc = lesson_data['subTopic']
                    sub_lesson = lesson_data['subLessonNumber']
                    sub_desc = lesson_data['subDescription']

                hist_entry = history(
                    histUserID = current_user.accountTableID,
                    histDueDate = datetime.today().date(),
                    histPageCurriculum = 'Jacqueline Ortiz ©The Little Ladybug Shop',
                    histLevel = 'U.S. Constitution Pennants',
                    histTopic = sub_topc,
                    histDescription = sub_desc,
                    histLesson = sub_lesson,
                    histQuestion = question,
                    histResponse = sub_answer
                )
                history_entries.append(hist_entry)

        """Add all history entries to the session"""
        db.session.add_all(history_entries)

        """Commit the session to save changes to the history table"""
        db.session.commit()
    ...
    filtered_hist_questions = []

    for lesson_row in adminLessonRow:
        lesson_num = lesson_row['subLessonNumber']
        subject_code = lesson_row['subSubjectCode'].casefold()
        if lesson_num in unique_subpx_lessons and subject_code == 'his' and lesson_num == lesson_number:
            filtered_hist_questions.append(lesson_row['subQuestion'])
    ...
    adminPicsTable = db.session.query(subjectPictures.subpxDateTime,
                                      ...
                                      subjectPictures.subpxLessonPic).filter(subjectPictures.subpxSubjectCode == 'HIS',
    ...
    response = make_response(render_template('histViewLesson.html',
                           ...
                           histQuestions = filtered_hist_questions,
...
@historyLesson_views.route('/histViewUploads//<int:lesson_number>', methods=['GET', 'POST'])
@login_required
def histViewUploads(lesson_number):
    """Upload"""
    if request.method == 'POST':
        ...
        histpxUserID = current_user.accountTableID

        """Assign to current time"""
        histpxDateTime = datetime.now()

        """Assign subject code"""
        histpxSubjectCode = 'HIS'

        """Set table"""
        upload = historyPics(
            histpxUserID = histpxUserID,
            histpxDateTime = histpxDateTime,
            histpxSubjectCode = histpxSubjectCode,
            histpxUploadPicA = fileA.read(),
            histpxUploadPicB = fileB.read(),
            histpxUploadPicC = fileC.read()
        )
    ...
    return render_template('histViewLesson.html', currentUser = current_user, lesson_number = lesson_number)